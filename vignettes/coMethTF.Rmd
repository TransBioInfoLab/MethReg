...
For promoter analysis, we recommend setting xx. For analysis of distal regions, 
we recommend xx. Note that the distal analysis will be more time and resource consuming. 
...
HOCOMOCO is a database of transcription factor binding sites (TFBS) motifs derived 
from _in vivo_ ChIP-seq datasets [@kulakovskiy2016hocomoco]. The function `get_tf_in_region` 
maps the regions to TF using a precomputed dataset of HOCOMOCO motifs . 
For each motif we scanned 500 bp around (i.e. +-250bp) each EPIC and HM450 arrays probes. 
Then for each motif found in a region we extract TF within the same Tf family or subfamily, 
using TFClass [@wingender2013tfclass], since they have similar binding motifs.

Therefore, a region is linked to a TF if any of the probes within it is mapped to the TF. 
In other words, we link the DNA methylation regions to TFs within 250 bp from the start or end of the region. 

```{R, results = "hide"}
# we will only map the first 200 regions, since this function may take a while to compute
region.tf <- get_tf_in_region(
    region = probes.gr[1:200], 
    genome = "hg38", 
    arrayType = "450k", 
...
Because TF binding to DNA can be influenced by (or influence) DNA methylation levels nearby (Yin et al. 2013), 
...
results.sig <- results[results$`pval_met:rna.tf`< 0.05 & results$`quant_pval_metGrp:rna.tf`< 0.05 ,]
```
...
```{R, plot_interaction_model, message = FALSE, results = "hide", eval = FALSE}
