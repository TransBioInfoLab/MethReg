...
## 4.3 Analysis using stratified model with methylation by TF interaction

To expand **Model 2** analysis, the function `stratified_model` 
will fit two separate models (shown below) to only the 
samples in highest DNAm levels (top 25 percent) then to only
the samples in lowest DNAm levels (bottom 25 percent).
...
![Possible scenarios](scenarios.png)



# 6. Identifying co-methylated clusters and summarizing regions using medians for DMRs

The R package [coMethDMR](https://github.com/TransBioInfoLab/coMethDMR) 
can be used to accurate identify co-methylated methylated regions [@gomez2019comethdmr].
The sections below will show how to use this package with the example dataset. 
