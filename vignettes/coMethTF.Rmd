...
author:
  ...
  - Lily Wang^[University of Miami Miller School of Medicine, lily.wang at gmail.com]
output: 
...
```{R}
dna.met.chr21.se <- make_se_from_dnam_probes(dna.met.chr21)
```
...
or (3) mapping the region to a specific number of genes upstream down/upstream of the region (`method = "nearest.genes"`)

For promoter analysis, we recommend setting `method = "closest.gene"`.
For analysis of distal regions, we recommend setting either `method = "window"` or `method = "nearest.genes"`. 
...
```{R memory_reduction, include = FALSE}
# Just reducing memory usage
probes.gr <- sort(probes.gr)[300:500,]
```
...
results.stratified.model <- stratified_model(
    triplet = triplet[1,],
    dnam = dna.met.chr21.se[triplet$regionID,],
    exp = gene.exp.chr21.se[c(triplet$target,triplet$TF),]
)
```
...
```{R, message = FALSE, results = "hide", include = FALSE}
results.stratified.model <- plot_stratified_model(
...
```{R, plot_stratified_model, message = FALSE, results = "hide", eval = FALSE}
