...
The function `interaction_model` will fit the following models to data:
...
2. by considering DNAm as a binary variable - we defined a binary group for
 ...
 are considered.
$$log_2(RNA target) \sim log_2(TF) + \text{DNAm Quantile Group} + log_2(TF) * \text{DNAm Quantile Group}$$
...
The following columns are provided for the full model:
...
The following columns are provided for the quartile model:
