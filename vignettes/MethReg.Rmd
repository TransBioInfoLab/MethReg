...
vignette: >
    %\VignetteIndexEntry{MethReg: estimating regulatory potential of DNA methylation changes in epigenome-wide association studies}
...
`make_dnam_se`. This function will use the Sesame package 
to map the array probes into genomic regions.
...
as those for CpGs. See **Section 6** "Identifying co-methylated clusters 
and summarizing regions using medians for DMRs" below for more details.   
...
(see **Section 7** "Controlling effects from confounding variables" below).
...
```{R}
gene.exp.chr21.se <- make_exp_se(gene.exp.chr21.log2)
gene.exp.chr21.se
...
To create the triplet dataset of CpGs, target genes and TFs, we first link 
each CpG with target gene (_CpG region-target gene_ dataset). 
Next, we link each CpG with TFs that have binding sites 
nearby (_CpG region-TF_ dataset). 
Finally, we merge the two datasets by CpGs. 

### Mapping Regions to the target gene

In this section, **regions** refer to the regions where CpGs are located. 
The function `get_region_target_gene` provides three different methods to 
...
of the region (`method = "nearby.genes"`) [@silva2019elmer].
...
of the region) (`method = "window"`) [@reese2019epigenome].
...
In this section, **regions** refer to the regions where CpGs are located. 
The function `get_tf_in_region` uses `motifmatchr` [@motifmatchr] to scan 
these regions for occurrences of motifs in the JASPAR database.
...
We create the triplet dataset by merging the _CpG region-target gene_ 
dataset and the _CpG region-TF_ dataset. 
...
plots <- plot_interaction_model(
    triplet.results = results.interaction.model[29,], 
    dnam = dna.met.chr21.se, 
    exp = gene.exp.chr21.se
)
```

```{R, fig.height = 8, fig.width = 10, eval = TRUE, fig.cap = "Plort example of a triplet result."}
plots
```
...
**_we can also nominate TF – target associations that might have been missed otherwise**_. 
...
```{r scenarios, fig.cap =  "Different biological scenarios considered by MethReg", echo = FALSE}
