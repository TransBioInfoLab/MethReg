...
get_tf_in_region <- function(region,
    ...
    # motifs.probes.list <- plyr::alply (motifs.probes[,1:2],.margins = 2,function(colum){names(colum)[which(colum > 0)]})
    # motifs.probes.df <- motifs.probes.list %>% plyr::ldply (data.frame)
    # colnames(motifs.probes.df) <- c("motif","region")

    motifs.tfs <- get(data(list = paste0("TF.",classification),package = "ELMER.data",envir = environment()))
    # motifs.tfs.df <- motifs.tfs %>% plyr::ldply (data.frame)
    # colnames(motifs.tfs.df) <- c("motif","TF_external_gene_name")
    # region.tf <- dplyr::full_join(motifs.probes.df,motifs.tfs.df)
    ...
    },.id = NULL,.progress = "time",.inform = TRUE,.parallel = parallel)
...
#' \dontrun{
#'  regions.names <- c("chr1:60591-79592","chr4:40162197-43162198")
#'  region.tf <- get_tf_in_region2(regions.names,
#'                  genome = "hg19",
#'                  arrayType = "450k",
#'                  classification = "subfamily")
#'
#'  regions.gr <- make_granges_from_names(regions.names)
#'  region.tf <- get_tf_in_region2(regions.gr,
#'                  genome = "hg38",
#'                  arrayType = "450k",
#'                  classification = "family")
#' }
...
#' @param cores Number of cores to be used. Default 1.
#' @importFrom Matrix colSums
get_tf_in_region2 <- function(region,
                              genome = c("hg19","hg38"),
                              arrayType = c("450k","EPIC"),
                              classification = c("subfamily","family"),
                              cores = 1) {
    ...
    motifs.probes.df$TF_ensembl_gene_id <- map_symbol_to_ensg(df$TF_external_gene_name)
