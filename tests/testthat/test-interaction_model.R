test_that("interaction_model handles 0 cases in more than 25% of samples with zeroinflated model", {
    ...
    expect_true("RLM_metGrp_pvalue" %in% colnames(results))
    expect_true("RLM_rna.tf_pvalue" %in% colnames(results))
    expect_true("RLM_metGrp_pvalue" %in% colnames(results))
    expect_true("RLM_metGrp_estimate" %in% colnames(results))
    expect_true("RLM_rna.tf_estimate" %in% colnames(results))
    expect_true("RLM_metGrp:rna.tf_estimate" %in% colnames(results))
})
