...
JASPAR database, we will then create a data frame 
...
However, oftentimes, the methylation data can also be, for example, **differentially methylated sites* (DMS) or **differentially methylated regions** (DMRs) obtained in an EWAS study.
...
`make_se_from_dnam_probes`. This funtion will use the Sesame package to map the array
...
```{R}
gene.exp.chr21 <- get(data("gene.exp.chr21", package = "coMethTF"))
...
1. Mapping the region to the closest gene (`method = "closest.gene"`)
2. Mapping the region to all the genes within a window 
(default size = 500 kbp around the region, i.e. +- 250 kbp from start or end of the region) (`method = "window"`).
3. Mapping the region to a specific number of genes upstream down/upstream of the region (`method = "nearest.genes"`)

For promoter analysis, we recommend setting `method = "closest.gene"`.
For the analysis of distal regions, we recommend setting either `method = "window"` or `method = "nearest.genes"`. 
...
region.target.nearest.genes <- get_region_target_gene(
    regions.gr = probes.gr, 
    genome = "hg38", 
...
Note that Model 1 partitions the effects of DNA methylation and TF on target gene expressions into three categories: 
...
results.sig <- results.interaction.model %>% 
  dplyr::filter(`pval_met:rna.tf` < 0.05 & `quant_pval_metGrp:rna.tf` < 0.05)
```
...
```{R, eval = TRUE, results = "hide", message = FALSE, eval = FALSE}
plots <- plot_stratified_model(
        triplet.results = results.stratified.model, 
        dnam = dna.met.chr21.se, 
        exp = gene.exp.chr21.se,
