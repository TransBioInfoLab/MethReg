test_that("interaction_model handles 0 cases in more than 25% of samples with zeroinflated model", {
    ...
    expect_true("RLM_DNAmGroup_pvalue" %in% colnames(results))
    expect_true("RLM_TF_pvalue" %in% colnames(results))
    expect_true("RLM_DNAmGroup_pvalue" %in% colnames(results))
    expect_true("RLM_DNAmGroup_estimate" %in% colnames(results))
    expect_true("RLM_TF_estimate" %in% colnames(results))
    expect_true("RLM_DNAmGroup:TF_estimate" %in% colnames(results))
})
